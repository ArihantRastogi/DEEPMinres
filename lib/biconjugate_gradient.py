...
#    @njit(parallel=True)
    ...
    def biconjugate_gradient(self, x_init, b, max_it=100, tol=1.0e-12, verbose=True):
        """
        Biconjugate Gradient method for solving Ax = b.
        Works for non-symmetric matrices as well.
        ...
        if verbose:
            print(f"Initial BiCG residual = {res}")
            
        if res < tol:
            if verbose:
                print(f"BiCG converged in 0 iterations. Final residual is {res}")
            return x, res_arr
        
        p = r.copy()
        p_tilde = r_tilde.copy()
        
        rho_prev = np.dot(r_tilde, r)
        
        for it in range(max_it):
            Ap = self.multiply_A_sparse(p)
            
            # Calculate A^T * p_tilde (transpose operation)
            AT_p_tilde = np.zeros_like(p_tilde)
            for i, j, v in zip(*self.A_sparse.transpose().nonzero(), self.A_sparse.transpose().data):
                AT_p_tilde[i] += v * p_tilde[j]
            
            alpha = rho_prev / np.dot(p_tilde, Ap)
            
            x = x + alpha * p
            r = r - alpha * Ap
            r_tilde = r_tilde - alpha * AT_p_tilde
            
            res = self.norm(r)
            res_arr.append(res)
            
            if verbose:
                print(f"Iteration {it+1}, residual = {res}")
                
            if res < tol:
                if verbose:
                    print(f"BiCG converged in {it+1} iterations to residual {res}")
                return x, res_arr
        ...
        if verbose:
            print(f"BiCG reached max iterations ({max_it}). Final residual is {res}")
            
        return x, res_arr

    def test_bicg_vs_cg(self, matrix_size=100, density=0.1, symmetric=False, seed=42):
        """
        Test BiConjugate Gradient vs normal CG on a sparse matrix.
        ...
        if symmetric:
            ...
            A = A + A.T + sparse.eye(matrix_size) * matrix_size
        else:
            # Create a non-symmetric matrix
            A = sparse.random(matrix_size, matrix_size, density=density, format='csr')
            # Make it diagonally dominant to ensure it's well-conditioned
            A.setdiag(np.abs(A).sum(axis=1) + 1)
        
        # Create a random solution
        x_true = np.random.rand(matrix_size)
        
        # Compute right-hand side
        b = A.dot(x_true)
        
        # Initialize solver with the test matrix
        solver = ConjugateGradientSparse(A)
        
        # Solve with BiCG
        x_init = np.zeros(matrix_size)
        print("\nSolving with BiConjugate Gradient method:")
        x_bicg, res_bicg = solver.biconjugate_gradient(x_init, b, verbose=True)
        
        # Solve with standard CG
        print("\nSolving with standard Conjugate Gradient method:")
        x_cg, res_cg = solver.cg_normal(x_init, b, verbose=True)
        
        # Compare solutions
        bicg_error = np.linalg.norm(x_bicg - x_true) / np.linalg.norm(x_true)
        cg_error = np.linalg.norm(x_cg - x_true) / np.linalg.norm(x_true)
        
        print(f"\nRelative error in BiCG solution: {bicg_error:.6e}")
        print(f"Relative error in CG solution: {cg_error:.6e}")
        
        # Plot convergence history
        plt.figure(figsize=(10, 6))
        plt.semilogy(res_bicg, label='BiCG')
        plt.semilogy(res_cg, label='CG')
        plt.xlabel('Iteration')
        plt.ylabel('Residual (log scale)')
        plt.title('Convergence Comparison: BiCG vs CG')
        plt.legend()
        plt.grid(True)
        plt.show()
        
        return x_bicg, x_cg, res_bicg, res_cg

# Add this to test the BiCG implementation when the file is run directly
if __name__ == "__main__":
    # Create a test sparse matrix and test BiCG vs CG
    print("Testing BiConjugate Gradient vs Conjugate Gradient methods")
    
    # Test on symmetric matrix
    print("\n--- Test on symmetric matrix ---")
    A_size = 100
    A_density = 0.05
    solver = ConjugateGradientSparse(sparse.eye(A_size))  # Temporary initialization
    solver.test_bicg_vs_cg(matrix_size=A_size, density=A_density, symmetric=True)
    
    # Test on non-symmetric matrix
    print("\n--- Test on non-symmetric matrix ---")
    solver.test_bicg_vs_cg(matrix_size=A_size, density=A_density, symmetric=False)
